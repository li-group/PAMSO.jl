...
function modgen0(n_loc,Location_u,Location,Location_tr,trline,Param,n_lij,p_val,plan_max)
	par_val = Dict()
	...
	function powerbal(m) #Function to add constraints on power balance
	    ...
	    @constraint(m,powbalru[t=1:n_tm],p_flowext[t]==sum(p_flow[(p,v),t] for (p,v) in trline if p in Location_u))
	    return m
	end
	m = powerbal(m)
	println("yes")
	...
	function transcon2(m) #Function to add constraints on transmision loss with peice wise linear power loss
	    ...
	    @constraint(m,tr3[(p,v) in trline,t=1:n_tm],-0.1*Fl_max[(p,v)]*n[(p,v)]*24*d_m[t]*per_val/S_base.<=0.1*p_flow[(p,v),t])
	    @constraint(m,tr4[(p,v) in trline,t=1:n_tm],0.1*p_flow[(p,v),t].<=0.1*Fl_max[(p,v)]*n[(p,v)]*24*d_m[t]*per_val/S_base)
	...
	function obj(m) #Function to objective function
	    ...
	    global eleccost = @expression(m,[t = 1:n_tm],sum(Ce[(t,k,h)]*w[(k,t)] for h in 1:24 for k in 1:n_k).*p_flowext[t]*S_base/(24*d_m[t]))
