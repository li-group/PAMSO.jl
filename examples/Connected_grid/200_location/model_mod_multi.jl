...
function modgen(n_loc,Location_u,Location,Location_tr,trline,Param,plan_max,n_lij,n_bun_agg)
	extract(Param)
	function variable_init() #Function to initialize model and variables
	    ...
	    @variable(m,Po_1[i in plant,loc in Location,1:n_tm,1:n_k,1:n_s])
	...
	function powerbal(m) #Function to add constraints on power balance
	    ...
	    @constraint(m,powbalru[t=1:n_tm,k=1:n_k,h=1:n_s],p_flowext[t,k,h]==sum(p_flow[(p,v),l,t,k,h] for l in 1:n_lij for (p,v) in trline if p in Location_u)) 
	    return m
	end
	m = powerbal(m)
	println("yes")
