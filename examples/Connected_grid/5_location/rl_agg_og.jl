...
function modgen0(n_loc,Location,Location_tr,trline,Param,n_lij,p_val)
	...
	function chemplant(m) #Function to add constraints relating to chemical plant
	   ...
	   @constraint(m,stoic[i in plant,mod1 in modes,c in chemical,c1 in chemical,loc in Location,t=1:n_tm],1000*F_1_mod[i,c,loc,mod1,t].*α[i,c1,mod1]./mw[(c,)].==1000*F_1_mod[i,c1,loc,mod1,t].*α[i,c,mod1]./mw[(c1,)])
	    ...
	    @constraint(m,minp[i in plant,mod1 in modes,loc in Location,t=1:n_tm],F_1_mod[i,Base_chem[i],loc,mod,t]>=y[i,loc,mod1,t].*C_min[(i,Base_chem[i],mod1)])
	    @constraint(m,maxp[i in plant,mod1 in modes,loc in Location,t=1:n_tm],F_1_mod[i,Base_chem[i],loc,mod,t]<=y[i,loc,mod1,t].*C_max[(i,Base_chem[i],mod1)])
