...
function modgen(n_loc,Location,Location_tr,trline,Param,plan_max,n_lij,n_bun_agg)
	...
	function variable_init() #Function to initialize model and variables
		...
		@variable(m,0<=y_1[i in plant,loc in Location,mod in modes,1:n_tm,1:n_k,1:n_s])
	    @variable(m,0<=z_1[i in plant,loc in Location,mod in modes,mod1 in modes,1:n_tm,1:n_k,1:n_s])
	    @variable(m,F_1[i in plant,c in chemical,loc in Location,1:n_tm,1:n_k,1:n_s])
	    @variable(m,F_1_mod[i in plant,c in chemical,loc in Location,mod in modes,1:n_tm,1:n_k,1:n_s])
	    ...
	    @variable(m,n_storage[i in storage,loc in Location]>=0,Int)
	    @variable(m,p_storage[i in storage,loc in Location,1:n_tm,1:n_k,1:n_s]>=0)
	    @variable(m,p_charge[i in storage,loc in Location,1:n_tm,1:n_k,1:n_s]>=0)
	    @variable(m,p_discharge[i in storage,loc in Location,1:n_tm,1:n_k,1:n_s]>=0)
	    return m
	...
	function chemplant(m) #Function to add constraints relating to chemical plant
	    ...
	    #@constraint(m,transdem[j in Consumer_supplier,c in c_jp[(j,)],t = 1:n_tm],sum(Tr_1[plant,c,Location,j,t])==D[(c,j,t)])
	...
	function powerbal(m) #Function to add constraints on power balance
	    ...
	    x = m[:x]
	    p_storage = m[:p_storage]
	   p_charge = m[:p_charge]
	   p_discharge = m[:p_discharge]
	   nso = m[:n_storage]
	    @variable(m,Gen[loc in Location,1:n_tm,1:n_k,1:n_s])
	    @variable(m,Dempl[loc in Location,1:n_tm,1:n_k,1:n_s])
	    @constraint(m,storcharge1[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_charge[i,loc,t,k,h]>=min_charge[i]*nso[i,loc]/S_base)
	    @constraint(m,storcharge2[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_charge[i,loc,t,k,h]<=max_charge[i]*nso[i,loc]/S_base)
	    @constraint(m,stordischarge1[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_discharge[i,loc,t,k,h]*S_base>=min_discharge[i]*nso[i,loc])
	    @constraint(m,stordischarge2[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_discharge[i,loc,t,k,h]*S_base<=max_discharge[i]*nso[i,loc])
	    @constraint(m,storage1[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_storage[i,loc,t,k,h]*S_base>=min_storage[i]*nso[i,loc])
	    @constraint(m,storage2[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_storage[i,loc,t,k,h]*S_base<=max_storage[i]*nso[i,loc])
	   #@constraint(m,storconn[loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_storage[loc,t,k,h]<=1)
	   @constraint(m,storcon1[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k],p_storage[i,loc,t,k,n_s]*S_base==0.5*max_storage[i]*nso[i,loc])
	     @constraint(m,storcon1_1[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k],p_storage[i,loc,t,k,1]==0.5*max_storage[i]*nso[i,loc]/S_base+p_charge[i,loc,t,k,1]*charge_efficiency[i]-p_discharge[i,loc,t,k,1]/discharge_efficiency[i])
	    @constraint(m,storcon2[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h=2:n_s],p_storage[i,loc,t,k,h]==p_storage[i,loc,t,k,h-1]+p_charge[i,loc,t,k,h]*charge_efficiency[i]-p_discharge[i,loc,t,k,h]/discharge_efficiency[i])
	    @constraint(m,storcon3[i in storage,loc in Location,t = 1:n_tm,k = 1:n_k,h = 1:n_s],p_charge[i,loc,t,k,h]<=Gen[loc,t,k,h]-p_cu[loc,t,k,h])
	    ...
	    @constraint(m,powbaln1[loc in Location,t=1:n_tm,k=1:n_k,h=1:n_s],p_cu[loc,t,k,h] + Dempl[loc,t,k,h] + sum(p_charge[:,loc,t,k,h]).==sum(p_discharge[:,loc,t,k,h]) + Gen[loc,t,k,h] -sum(p_flow[(p,v),l,t,k,h] for l in 1:n_lij for (p,v) in trline if p==loc)) 
	    ...
	    @constraint(m,powbalru1[t=1:n_tm,k=1:n_k,h=1:n_s],p_flowext[t,k,h]==0)
	    return m
	end
	m = powerbal(m)
	println("yes")
	...
	function obj(m) #Function to objective function
	    ...
	    global CAP_st = @expression(m,[i in storage],sum(DIC[(i)].*nso[i,:])/(n_m))
	    if(length(trline)==0)
	    	global FIXOP_l = 0
	    else
	    ...
	    @objective(m,Min,-(sum(matcostc)-sum(transcostc)-sum(eleccost)-sum(FIXOP)-sum(FIXOP_l)-sum(CAPEX)-sum(CAPEX_l)-sum(CAP_st))/1000)
	    return m
	end
	m = obj(m)
	return m
