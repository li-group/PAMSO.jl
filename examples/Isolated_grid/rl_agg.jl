...
function modgen0(n_loc,Location,Location_tr,trline,Param,n_lij,p_val)
    ...
    Min_inv[("Solar panel")] = 0
    Min_inv[("Wind Turbine")] = p_val[1]
	...
	function xmax(m) #Function to add constraints on x
	    ...
	    @constraint(m,sum(x["Wind Turbine",Location])>=Min_inv["Wind Turbine"])
	    @constraint(m,sum(x["Solar panel",Location])>=Min_inv["Solar panel"])
	...
	function chemplant(m) #Function to add constraints relating to chemical plant
	    ...
	    @constraint(m,powerel[i in plant,loc in Location,t=1:n_tm],0.1*Po_1[i,loc,t]*1==sum(0.1*F_1_mod[i,Base_chem[i],loc,mod1,t]*f_lin[(i,mod1)] for mod1 in modes)+sum(0.1*F_1[i,c,loc,t].*F_comp[(i,c)].*S[(i,c)] for c in chemical))
