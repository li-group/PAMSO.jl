...
function gendata(n_bun,trline,Location,dist)
	...
	max_discharge = Dict()
	min_discharge = Dict()
	charge_efficiency = Dict()
	discharge_efficiency = Dict()
	max_storage = Dict()
	min_storage = Dict()
	...
	for i in files_power
	 ...
	 for i in files_storage
		...
		for i =1:nrow(df_data)
        	...
        	if(param[i]=="max_charge")
        		max_charge[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="min_charge")
        		min_charge[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="max_discharge")
        		max_discharge[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="min_discharge")
        		min_discharge[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="charge_efficiency")
        		charge_efficiency[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="discharge_efficiency")
        		discharge_efficiency[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="max_storage")
        		max_storage[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="min_storage")
        		min_storage[df_data[i,2]] = df_data[i,3]
        	end
        	if(param[i]=="tl")
        		tl[df_data[i,2]] = df_data[i,3]
        	end
	...
	for i in storage
		If = 1
	  	DIC[(i)] = OCC[(i)]*ir*If/(1-1/(1+ir)^tl[(i)])
	end
	DIC_l = Dict()
