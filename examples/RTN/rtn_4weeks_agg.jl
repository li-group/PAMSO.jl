...
function func_all(x)
    ...
    for i in 1:weeks
        ...
        data = create_model_data(task, resources, network, supply, horizon,days,num_day_week)
        RTN_model= RTN(data)
        opt = solve_model_full(RTN_model,V_max,X_max,"agg_results_week_"*string(i)*".xlsx")
        #opt = solve_model_full(RTN_model)
        opt_val = opt_val+opt
    end
    
    outp = DataFrame(A = opt_val,B = x[1],C = x[2],D = x[3],E = x[4],F = x[5],G = x[6],H = Dates.format(now(), "e: dd u yy, HH.MM.SS"))
    CSV.write("agg_4weeks.csv",outp,append = true)
    
    
    # Create DataFrame
    return opt_val/weeks
end
func_all([0.635,24,16.869,29.308,27.853,0.512])
