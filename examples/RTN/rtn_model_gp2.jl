...
function solve_model_full(rtn::RTN,Vmaxval = Nothing,Xmaxval = Nothing,xlsxfile=Nothing)
   ...
   println(Dem)
    ...
    sl_val = value.(sl)
    df_sl = DataFrame(A = "F",B = 1,C=sl_val["F",1])
    for r in Rp
        for n in Td
            
                push!(df_sl,[r,n,sl_val[r,n]])
        
        end
    end
    Vmax_val = value.(Vmax)
    df_v = DataFrame(A = "V_1",B = Vmax_val["V_1"])
    for r in Task_resources
        
            push!(df_v,[r,Vmax_val[r]])
      
    end


    Xmax_val = value.(Xmax_val)
    df_x = DataFrame(A = "A",B = Xmax_val["A"])
    for r in R_mat

            push!(df_x,[r,Xmax_val[r]])
    ...
    sheet_names = ["Sl", "Vmax","Xmax"]
    XLSX.writetable(xlsxfile, overwrite=true, 
    sl=(collect(DataFrames.eachcol(df_sl)), DataFrames.names(df_sl)),
    v=(collect(DataFrames.eachcol(df_v)), DataFrames.names(df_v)),
    x = (collect(DataFrames.eachcol(df_x)), DataFrames.names(df_x))
